        ...
        <a data-toggle="modal" data-target="#aside" class="hidden-lg-up mr-3">
          ...
          <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content">
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-6">
                        <form action="{{ route('add_email_photoshop') }}" method="post">
                            @csrf
                            <div class="form-group">
                                <label for="exampleInputEmail">إضافة بريد أكاديمي photoshop جديد</label>
                    ...
                    <div class="col-md-6">
                        <form action="{{ route('emailExcel_photo') }}" method="POST"  enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
            ...
            <h1> <b>البريد الأكاديمي photoshop</b></h1>
            <table class="table">
                        ...
                        <tr>
                            ...
                            <td>@if($email->appointment_date)
                             ...
                             <td>
                                ...
                                <form action="{{ route('emails_photoshop.destroy', $email->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
               ...
               @if($isTableNotEmpty)
                ...
                @if (isset($confirmation))
                    ...
                    <form method="post" action="{{ route('deletePhoto_email') }}">
                        @csrf
                ...
                <div class="modal fade" id="editModal{{$email->id}}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{$email->id}}" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form action="{{ route('emails_photoshop.update', $email->id) }}" method="POST">
                                @csrf
                                @method('PUT')
...
<script>
 function confirmDelete() {
            if (confirm("Are you sure you want to delete the table? This action cannot be undone.")) {
                window.location.href = "{{ route('deletePhoto_email', ['confirmation' => true]) }}";
            }
        }
        </script>
</body>
