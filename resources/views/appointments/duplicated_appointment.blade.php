        ...
        <a data-toggle="modal" data-target="#aside" class="hidden-lg-up mr-3">
          ...
          <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content">
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-6">
                        <form action="{{ route('add_appointment_duplicated') }}" method="post">
                            @csrf
                            <div class="form-group">
                                <label for="exampleInputappointment">إضافة موعد Duplicated جديد</label>
                    ...
                    <div class="col-md-6">
                        <form action="{{route('appointmentExcel_duplicated')}}" method="POST"  enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
            ...
            <table class="table">
                        ...
                        <tr>
                            ...
                            <td>{{ $appointment->user_count }}</td>
                             <td>
                                ...
                                <form action="{{ route('appointments_duplicated.destroy', $appointment->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
               ...
               @if($isTableNotEmpty)
                ...
                @if (isset($confirmation))
                    ...
                    <form method="post" action="{{ route('deleteDuplicated_appointment') }}">
                        @csrf
                ...
                <div class="modal fade" id="editModal{{$appointment->id}}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{$appointment->id}}" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form action="{{ route('appointments_duplicated.update', $appointment->id) }}" method="POST">
                                @csrf
                                @method('PUT')
...
<script>
 function confirmDelete() {
            if (confirm("Are you sure you want to delete the table? This action cannot be undone.")) {
                window.location.href = "{{ route('deleteDuplicated_appointment', ['confirmation' => true]) }}";
                return true;
            }
            return false;
        }
