        ...
        <a data-toggle="modal" data-target="#aside" class="hidden-lg-up mr-3">
          ...
          <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content">
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-6">
                        <form action="{{ route('add_appointment_design') }}" method="post">
                            @csrf
                            <div class="form-group">
                                <label for="exampleInputappointment">إضافة موعد Design جديد</label>
                    ...
                    <div class="col-md-6">
                        <form action="{{route('appointmentExcel_design')}}" method="POST"  enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
            ...
            <table class="table">
                        ...
                        <tr>
                            ...
                            <td>{{ $appointment->user_count }}</td>
                             <td>
                                ...
                                <form action="{{ route('appointments_design.destroy', $appointment->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
               ...
               @if($isTableNotEmpty)
                ...
                @if (isset($confirmation))
                    ...
                    <form method="post" action="{{ route('deleteDesign_appointment') }}">
                        @csrf
                ...
                <div class="modal fade" id="editModal{{$appointment->id}}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{$appointment->id}}" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form action="{{ route('appointments_design.update', $appointment->id) }}" method="POST">
                                @csrf
                                @method('PUT')
...
<script>
 function confirmDelete() {
            if (confirm("Are you sure you want to delete the table? This action cannot be undone.")) {
                window.location.href = "{{ route('deleteDesign_appointment', ['confirmation' => true]) }}";
                  return true;
            }
              return false;
        }
